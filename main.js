                ...
                for (var i = 0; i < mutations.length; ++i) {
                    for (var j = 0; j < mutations[i].addedNodes.length; ++j) {
                        ...
                        if ($(mutations[i].addedNodes[j]).hasClass('error-dialog')) {
                            ...
                            // on contextmenu (right-click) hide dialog, instead of loading image
                            if (event.type === 'contextmenu') {
                                dialog.modal('hide');
                            } else {
                                dialog.find('h1').text(filename);
                                dialog.find('.modal-body').html('<img src="file://' + filepath +
                                    '" style="max-width: 100%; max-height: 300px; border-radius: 5px; margin: 20px auto 0; display: block;" />');
                                dialog.find('img').on("load", function() {
                                    $(this).after(
                                        '<p style="text-align: center; margin: 10px 0 15px; font-size: 120%;">' +
                                        this.naturalWidth + ' x ' + this.naturalHeight + ' ' +
                                        Strings.UNIT_PIXELS + '</p>');
                                });
                            }
